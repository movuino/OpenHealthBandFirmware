...
//write_reg(REG_MODE_CONFIG, 0b00000110);

      // Configure FIFO.

         
       //write_reg(REG_FIFO_CONFIG_1, 0b00010000); //(A_FULL = 16)
       //write_reg(REG_FIFO_CONFIG_1, 128-116); //(A_FULL = 12)
...
/*for(int i=0; i<sample_count*3;i++){
 ...
 tagSeq1A_PD1 = tagSeq1A_PD11[0];
 ledSeq1A_PD1 = ledSeq1A_PD11[0];
 tagSeq1B_PD1 = tagSeq1B_PD11[0];
 ledSeq1B_PD1 = ledSeq1B_PD11[0];
   ...
   /* for (i = 0; i < sample_count/2; i++)
    {
        ...
        tagSeq1A_PD22[i] = (dataBuf[j*3+0] >> 3) & 0x1f;
 ...
 int *tagSeq1A_PD22 = (int*)malloc((sample_count/4)*sizeof(int)),*ledSeq1A_PD22 = (int*)malloc((sample_count/4)*sizeof(int)),*tagSeq1B_PD22 = (int*)malloc((sample_count/4)*sizeof(int)),*ledSeq1B_PD22 = (int*)malloc((sample_count/4)*sizeof(int));
   ...
   /////////////////////////////////////
    ...
    {   
        ...
        //Serial.println(ledSeq1A_PPG11[i]);

       }
         else{

        tagSeq1B_PD11[i] = (dataBuf[i*12+0] >> 3) & 0x1f;
        ledSeq1B_PD11[i] = ((dataBuf[i*12+0] << 16) | (dataBuf[i*12+1] << 8) | (dataBuf[i*12+2])) & 0x7ffff;

        tagSeq1B_PD22[i] = (dataBuf[i*12+3] >> 3) & 0x1f;
        ledSeq1B_PD22[i] = ((dataBuf[i*12+3] << 16) | (dataBuf[i*12+4] << 8)| (dataBuf[i*12+5])) & 0x7ffff;

        tagSeq1A_PD11[i] = (dataBuf[i*12+6] >> 3) & 0x1f;
        ledSeq1A_PD11[i] = ((dataBuf[i*12+6] << 16) | (dataBuf[i*12+7] << 8) | (dataBuf[i*12+8])) & 0x7ffff;

        tagSeq1A_PD22[i] = (dataBuf[i*12+9] >> 3) & 0x1f;
        ledSeq1A_PD22[i] = ((dataBuf[i*12+9] << 16) | (dataBuf[i*12+10] << 8) | (dataBuf[i*12+11])) & 0x7ffff;
         }
    ...
    /*Serial.print("Hz[");
    Serial.print((float)samplesTaken1 / ((millis() - startTime1) / 1000.0), 2);
    Serial.print("]");
    Serial.println();*/
  ...
  tagSeq1B_PD2 = tagSeq1B_PD22[0];
  ledSeq1B_PD2 = ledSeq1B_PD22[0];
