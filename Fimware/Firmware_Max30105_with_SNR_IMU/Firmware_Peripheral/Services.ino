void setupPPGService(void)
{
  ...
  snrPPGCharacteristic.setProperties(CHR_PROPS_NOTIFY);
  snrPPGCharacteristic.setPermission(SECMODE_OPEN, SECMODE_NO_ACCESS);
  snrPPGCharacteristic.setFixedLen(4);
  snrPPGCharacteristic.setCccdWriteCallback(cccd_callback_rawPPG);  // Optionally capture CCCD updates
  snrPPGCharacteristic.begin();
}
