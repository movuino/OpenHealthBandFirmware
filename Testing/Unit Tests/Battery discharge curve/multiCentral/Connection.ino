...
*/
  ...
  if (SNR1_2_Characteristic2.enableNotify() )
    ...
    Serial.println("Couldn't enable notify SNR1_2_Characteristic2 characteristic. Increase DEBUG LEVEL for troubleshooting");
     ...
     if ( !SNR2_2_Characteristic2.discover() )
    {
    // Measurement chr is mandatory, if it is not found (valid), then disconnect
    Serial.println("SNR2_2_Characteristic2 characteristic not found !!!");  
    Bluefruit.disconnect(conn_handle);
    return;
    }
  Serial.println("SNR2_2_Characteristic2 characteristic found");
  delay(20);
  // Reaching here means we are ready to go, let's enable notification on measurement chr
  if (SNR2_2_Characteristic2.enableNotify() )
    {
    Serial.println("Ready to receive SNR2_2_Characteristic2 characteristic data");
    }
  else
    {
