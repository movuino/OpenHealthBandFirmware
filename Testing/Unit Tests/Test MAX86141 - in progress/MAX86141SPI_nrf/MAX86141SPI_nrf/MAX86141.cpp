...
uint8_t MAX86141::read_reg(uint8_t address) {
    ...
    // m_rx_buf[0] = spi->transfer(0x00);
    // m_rx_buf[1] = spi->transfer(0x00);
    // m_rx_buf[2] = spi->transfer(0x00);
...
void MAX86141::init(int newSpiClk=10000)
{
    setSpiClk(newSpiClk);
    uint8_t temp;
    ...
    write_reg(REG_LED_RANGE_1, 0b11111111); // xx,LED3,LED2,LED1. 00,01,10,11 low to high
    write_reg(REG_LED1_PA, 20); // 0 = 0mA, 255 = Max mA
    write_reg(REG_LED2_PA, 20);
    ...
    // note to self: figure this part out

    //write_reg(REG_LED_SEQ_1, 0x21);
    //write_reg(REG_LED_SEQ_2, 0x05);
    write_reg(REG_LED_SEQ_1, 0b01001001); //DATA BUF 2 | DATA BUF 1  // 0001 - LED 1, 0010 - LED2, 0011 - LED3, 1001 - AMBIENT
    write_reg(REG_LED_SEQ_2, 0b00010010); //DATA BUF 4 | DATA BUF 3  //
...
void MAX86141::device_data_read(void)
{
    ...
    uint8_t dataBuf[1536]; ///128 FIFO samples, 2 channels, 1 PDs, 3 bytes/channel 128*3*2*3 = 2304 byte buffer
