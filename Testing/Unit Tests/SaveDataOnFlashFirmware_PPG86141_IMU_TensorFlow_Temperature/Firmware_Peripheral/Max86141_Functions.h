...
#ifdef Flash
    Serial.println("ADDITION DES DATA");
    uint8_t data_sum[20] = {0};
    
    for (int i = 0; i < 20; i++) {
      data_sum[i] = ptledSeq1APD1[i];
    }

    for (int i = 0, j = 20; i < 20, j < 40; i++, j++) {
      data_sum[j] = ptledSeq1APD2[i];
    }

    /* Write data on Flash */
    write_Flash(data_sum);
    
    /* Read data on Flash */
    read_Flash();
    
    for (int i = 0; i < 40; i++) {
      Serial.print(data_read_from_Flash[i]);
      Serial.print("--> ");
    }
    Serial.println();
