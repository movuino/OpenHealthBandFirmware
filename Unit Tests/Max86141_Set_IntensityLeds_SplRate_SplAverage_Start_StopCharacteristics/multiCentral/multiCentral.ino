...
BLEClientCharacteristic intensityLedsCharacteristic(0x1315);
BLEClientCharacteristic smplRateCharacteristic(0x1316);
BLEClientCharacteristic smplAvgCharacteristic(0x1317);
...
void loop() {
  while (Serial.available() > 0) {
    ...
    inData += recieved;
  /* Format input data : 1,010,14,0 where 1 is start sending, 010 is the intensity, 14 the sample rate (= 128 sps) and 0x0 the sample average (= 1)*/
    if (recieved == '\n') {
      /*Started = inData.substring(0,1).toInt();
      intensity_leds = inData.substring(2,5).toInt();
      smpl_rate = inData.substring(6,8).toInt();
      smpl_avg =  inData.substring(9).toInt();*/
