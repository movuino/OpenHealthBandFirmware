      ...
      const BLE_ACC_GYR_MAG_CHARACTERISTIC_UUID = 0x1102;

      const BLE_PPG86_SERVICE_UUID = 0x1300;
      ////// PDLEDs ////////

      ////// PDsLEDs ////////
      const BLE_PDsLEDsCHARACTERISTIC2_UUID = 0x1303;
      ...
      var acc_resolution = 0.00049;
      var gyro_resolution = 0.06;
      var mag_resolution = 1.50;
      var mag_bias_factory_0 = 1.18, mag_bias_factory_1 = 1.19, mag_bias_factory_2 = 1.14;
      var mag_scale_0 = 1.00, mag_scale_1 = 1.00, mag_scale_2 = 1.00;
      var mag_bias_0 = 0.00, mag_bias_1 = 0.00, mag_bias_2 = 0.00;
      var bias_to_current_bits = 1.00;
      ...
      try{
        ...
        document.getElementById("PPG-state").classList = PPG86Error ? "status-ko" : "status-ok";
         ...
         var av = (value.getInt8(4)<<8) | value.getInt8(5);
         var ax = (av) * acc_resolution;

          av = (value.getInt8(6)<<8) | value.getInt8(7);
         var ay = (av) * acc_resolution;

          av = (value.getInt8(8)<<8) | value.getInt8(9);
         var az = (av) * acc_resolution;
        

        IMUAccXLine.append(new Date().getTime(), ax)
        IMUAccYLine.append(new Date().getTime(), ay)
        IMUAccZLine.append(new Date().getTime(), az)

        var v1 = (value.getInt8(10)<<8) | value.getInt8(11);

        var gx = (v1) * gyro_resolution;

        v1 = (value.getInt8(12)<<8) | value.getInt8(13);
        var gy = (v1) * gyro_resolution;

        v1 = (value.getInt8(14)<<8) | value.getInt8(15);
        var gz = (v1) * gyro_resolution;
        
          IMUGyrXLine.append(new Date().getTime(), gx)
          IMUGyrYLine.append(new Date().getTime(), gy)
          IMUGyrZLine.append(new Date().getTime(), gz)

        var mx = (((value.getInt8(17)<<8) | value.getInt8(16)) * mag_resolution * mag_bias_factory_0 - mag_bias_0 * bias_to_current_bits) * mag_scale_0;
        var my = (((value.getInt8(19)<<8) | value.getInt8(18)) * mag_resolution * mag_bias_factory_1 - mag_bias_1 * bias_to_current_bits) * mag_scale_1;
        var mz = (((value.getInt8(21)<<8) | value.getInt8(20)) * mag_resolution * mag_bias_factory_2 - mag_bias_2 * bias_to_current_bits) * mag_scale_2;

       //console.log(x,y,z)
        IMUMagXLine.append(new Date().getTime(), mx)
        IMUMagYLine.append(new Date().getTime(), my)
        IMUMagZLine.append(new Date().getTime(), mz)
                  }
                    );
                ...
                if(characteristic.uuid== '00001303-0000-1000-8000-00805f9b34fb'){
      ...
      async function LEDSEQ1A_PD1CHARACTERISTIC2Changed(event) {
        ...
        const timestamp = (value.getUint8(3) << 24) | (value.getUint8(2) << 16) | (value.getUint8(1) << 8) | value.getUint8(0); // shifts 4 Uint8 to make a Uint32
        
        const data1PD1 = (value.getUint8(7) << 24) | (value.getUint8(6) << 16) | (value.getUint8(5) << 8) | value.getUint8(4); // shifts 4 Uint8 to make a Uint32
        const data2PD1 = (value.getUint8(11) << 24) | (value.getUint8(10) << 16) | (value.getUint8(9) << 8) | value.getUint8(8); // shifts 4 Uint8 to make a Uint32

        console.log("data PD1");
        console.log("data1PD1: ", data1PD1,"data2PD1: ", data2PD1);

        var tab;
        tab = [data1PD1,data2PD1]
        ...
        var snr1_2PD1CHARACTERISTIC2 = (value.getUint8(15) << 24) | (value.getUint8(14) << 16) | (value.getUint8(13) << 8) | value.getUint8(12); // shifts 4 Uint8 to make a Uint32
       ...
       document.getElementById('snr1_2PD1').innerHTML = String(snr_neg);
        ...
        const data1PD2 = (value.getUint8(19) << 24) | (value.getUint8(18) << 16) | (value.getUint8(17) << 8) | value.getUint8(16); // shifts 4 Uint8 to make a Uint32
        const data2PD2 = (value.getUint8(23) << 24) | (value.getUint8(22) << 16) | (value.getUint8(21) << 8) | value.getUint8(20); // shifts 4 Uint8 to make a Uint32
        
          //console.log("timestamp: ", timestamp);
        console.log("data PD2")
        console.log("data1PD2: ", data1PD2,"data2PD2: ", data2PD2);

        var tab;
        tab = [data1PD2,data2PD2]
          for (var i = 0; i<2; i++) {
            if(tab[i]!=0){
          ...
          const snr1_2PD2CHARACTERISTIC2 = (value.getUint8(27) << 24) | (value.getUint8(26) << 16) | (value.getUint8(25) << 8) | value.getUint8(24); // shifts 4 Uint8 to make a Uint32
