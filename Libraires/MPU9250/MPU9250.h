...
private:
    ...
    void read_mag(int16_t* destination) {
        if (read_byte(AK8963_ADDRESS, AK8963_ST1) & 0x01) {                  // wait for magnetometer data ready bit to be set
            read_bytes(AK8963_ADDRESS, AK8963_XOUT_L, 7, &raw_data_mag[0]);      // Read the six raw data and ST2 registers sequentially into data array
            uint8_t c = raw_data_mag[6];                                         // End data read by reading ST2 register
            if (!(c & 0x08)) {                                               // Check if magnetic sensor overflow set, if not then report data
                destination[0] = ((int16_t)raw_data_mag[1] << 8) | raw_data_mag[0];  // Turn the MSB and LSB into a signed 16-bit value
                destination[1] = ((int16_t)raw_data_mag[3] << 8) | raw_data_mag[2];  // Data stored as little Endian
                destination[2] = ((int16_t)raw_data_mag[5] << 8) | raw_data_mag[4];
            }
        }
    }
